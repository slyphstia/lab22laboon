...
int Unit :: ultimateAttack(Unit &oppulti){
	return oppulti.beAttacked(atk*2) ;
}
...
void Unit :: equip(Equipment *c){
	...
	vector<int> statn = c->getStat();
	if(equipment != NULL){	
		vector<int> stato = equipment->getStat();
		stato =equipment->getStat();
		statn = c->getStat();
		hpmax += statn[0]-stato[0];
		atk += statn[1]-stato[1];
		def += statn[2]-stato[2]; 
	}
	else{
